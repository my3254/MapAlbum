...
import { Image as ImageIcon, MapPin, Plus, Star, Upload, X } from 'lucide-react';
        ...
        <>
          ...
          <section className="inspector__section">
            ...
            <div className={`chip-grid${selectedAlbum ? ' chip-grid--compact' : ''}`}>
              {stagedImages.length === 0 && <p className="muted-line">暂时还没有选择图片。</p>}
                ...
                {albumImages.map((imagePath) => {
                  ...
                  return (
                    <figure key={imagePath} className="photo-card" style={{ position: 'relative' }}>
                      <img src={toLocalMediaUrl(imagePath)} alt={selectedAlbum.displayName} />
                      {!isCover && (
                        <button
                          className="icon-button icon-button--small"
                          title="设为封面"
                      ...
                      {isCover && (
                        <div style={{ position: 'absolute', top: 8, right: 8, padding: '4px 8px', background: 'var(--accent)', color: '#000', borderRadius: 6, fontSize: 11, fontWeight: 'bold' }}>
                          封面
                        </div>
                      )}
                    </figure>
